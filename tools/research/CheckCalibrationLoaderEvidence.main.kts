...
require(args.size==2){"Usage: CheckCalibrationLoaderEvidence.main.kts RUN_DIRECTORY OUTPUT_DIRECTORY"}
...
val verifier=listOf(run.resolve("source/VerifyCalibrationLoaders.main.kts"),Path.of("tools/guest/VerifyCalibrationLoaders.main.kts").toAbsolutePath()).first{Files.isRegularFile(it)}
...
data class Case(val name:String,val events:String=source,val damage:String?=null,val pass:Boolean=false)
val cases=listOf(Case("accepted",pass=true),Case("wrong-signed-status",flip(source,"loader-status",0,"actual")),Case("missing-debug-ready",remove(source,"loader-debug-ready",1)),Case("changed-checkpoint-register",flip(source,"loader-checkpoint-registers",2,"x15")),Case("missing-reap",remove(source,"group-reaped",0)),Case("wrong-prefix-write",flip(source,"modeled-write",100,"value")),Case("damaged-vertical-file",damage="loader-76-terminal-vertical.bin"))
