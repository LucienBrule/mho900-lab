...
run_id=${1:?Usage: run-baseline.sh RUN_ID}
...
"$timeout_bin" 330 "$@" > "$run/emulator.log" 2>&1 &
emulator_pid=$!
boot=timed_out
install=not_reached
launch=not_reached
...
while [ "$(date +%s)" -lt "$deadline" ]; do
    ...
    if [ "$state" = device ]; then
        if [ -z "$logcat_pid" ]; then
            "$timeout_bin" 300 "$sdk/platform-tools/adb" -P 5041 -s emulator-5580 \
...
if [ "$boot" = completed ]; then
    ...
    install_rc=0
    "$timeout_bin" 30 "$sdk/platform-tools/adb" -P 5041 -s emulator-5580 install "$apk" \
        > "$run/install.txt" 2>&1 || install_rc=$?
    install=failed
    if [ "$install_rc" = 124 ]; then install=timed_out; fi
    if [ "$install_rc" = 0 ] && grep -q '^Success' "$run/install.txt"; then
        install=admitted
        launch_rc=0
        adb shell am start -W -n com.rigol.scope/.SplashActivity > "$run/launch.txt" 2>&1 || launch_rc=$?
        launch=attempted
        if [ "$launch_rc" = 124 ]; then launch=timed_out; fi
        sleep 15
        adb shell pidof com.rigol.scope > "$run/app-pid.txt" 2>&1 || true
        adb shell dumpsys activity activities > "$run/activities.txt" 2>&1 || true
        adb exec-out screencap -p > "$run/application.png" 2> "$run/app-screenshot-error.txt" || true
...
logcat_pid=
trap - EXIT
for artifact in "$run"/*.txt "$run"/*.log "$run"/*.png "$run"/result.toml; do
    [ "$artifact" != "$run/evidence-sha256.txt" ] || continue
    [ -f "$artifact" ] && shasum -a 256 "$artifact"
