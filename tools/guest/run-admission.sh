...
run_id=${1:?Usage: run-admission.sh RUN_ID [inspect|probe|label|startup|mapping|syscall|native|exclusive|execution]}
mode=${2:-inspect}
case "$mode" in inspect|probe|label|startup|mapping|syscall|native|exclusive|execution) ;; *) exit 2;; esac
