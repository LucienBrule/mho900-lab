...
run_id=${1:?Usage: run-admission.sh RUN_ID [inspect|probe|label|startup|mapping|syscall|native|exclusive]}
mode=${2:-inspect}
case "$mode" in inspect|probe|label|startup|mapping|syscall|native|exclusive) ;; *) exit 2;; esac
