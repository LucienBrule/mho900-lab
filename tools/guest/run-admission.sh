...
run_id=${1:?Usage: run-admission.sh RUN_ID [inspect|probe|label|startup|mapping|syscall|native|exclusive|execution|threads|discovery|coverage|groupcontrol|groupmodel]}
mode=${2:-inspect}
case "$mode" in inspect|probe|label|startup|mapping|syscall|native|exclusive|execution|threads|discovery|coverage|groupcontrol|groupmodel) ;; *) exit 2;; esac
...
if [ "$boot" = completed ]; then
    ...
    helper_mode=$mode
    case "$mode" in startup|mapping|syscall|native|coverage|groupmodel) helper_mode=label;; esac
