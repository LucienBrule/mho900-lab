...
if [ "$mode" = filesystem ]; then
    ...
    cp "$fixture_manifest" "$run/source/calibration-filesystem-inputs.toml"
...
if [ "$mode" = filesystem ]; then set -- "$@" -ramdisk "$run/fixture-ramdisk.img"; fi
...
if [ "$boot" = completed ]; then
    ...
    case "$mode" in loadercontrol|loaderisolated|filesystem)
        health_rc=0
