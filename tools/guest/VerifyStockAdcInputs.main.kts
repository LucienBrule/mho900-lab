...
require(args.size==1){"Usage: VerifyStockAdcInputs.main.kts RUN_DIRECTORY"}
...
val captureSourceSha256 = "f04e3bedadbecdba54a4e3ab590b748ff88d3d1c421f06315e868b51a6dbe261"
val captureHeaderSha256 = "78320d758756a0c7483bd29caf916251e93ab66b5a3883c4415e3cce0d4fd548"
val captureInputHeaderSha256 = "086b2f0bb35bf340a7b4fb37e112d3b648c618e85043795c00014508b4b77b82"
require(captureNativeSha256.length == 64 && captureSourceSha256.length == 64 && captureHeaderSha256.length == 64) { "capture freeze pins unresolved" }
...
staticPins.forEach { (name, digest) -> require(hash("adc-parameter-static/$name") == digest) }
require(hash("source/adc-input-capture-decision.toml") == "49554ca20c721623d947b42c211948a2ba2f91d32d9b1c27a36c06fb3f617a9f")
require(hash("source/adc-parameter-capture.h")==captureInputHeaderSha256)
require(hash("source/group-observer.c")==captureSourceSha256)
require(hash("source/calibration-loaders.h")==captureHeaderSha256)
...
val stockInputs=text("source/adc-input-capture-inputs.toml")
...
require(field(nativeInput,"source_sha256")==captureSourceSha256)
require(field(nativeInput,"header_sha256")==captureHeaderSha256)
require(field(nativeInput,"capture_header_sha256")==captureInputHeaderSha256)
...
for ((name,digest) in mapOf("group-observer.c" to captureSourceSha256,"calibration-loaders.h" to captureHeaderSha256,"adc-parameter-capture.h" to captureInputHeaderSha256)) {
    val records=buildInputs.lineSequence().filter{it.endsWith("/$name")}.toList()
    require(records.size==1&&records.single().substringBefore(" ")==digest){"native build source: $name"}
}
...
expectFields(stockInputs.substringBefore("\n["), mapOf(
    "mode" to "\"adcinputmodel\"", "run_id" to "\"stock-adc-input-capture-01\"",
...
fun verify(){
 ...
 if(reads.isNotEmpty())require(es.indexOf(befores.first())<es.indexOf(reads.first()))
    ...
    run {
        val builder=ProcessBuilder("kotlin",run.resolve("source/VerifyAdcInputCapture.main.kts").toString(),run.toString()).redirectErrorStream(true)
        builder.environment().remove("KOTLIN_RUNNER");val process=builder.start();val output=process.inputStream.bufferedReader().readText()
        require(process.waitFor()==0){"VerifyAdcInputCapture failed: $output"};System.err.print(output)
    }
    ...
    val consumed=hostCaptureNames+listOf("source/VerifyStockAdcInputs.main.kts","source/VerifyAdcInputCapture.main.kts","source/adc-input-capture-inputs.toml","source/calibration-stock-runtime-inputs.toml","adc-input-maps.txt","adc-input-matrix.bin","adc-input-setting.bin","adc-input-drvparam.bin","adc-input-series.bin","adc-input-config.bin","adc-input-sample-entry.bin","adc-input-shadow-low.bin","adc-input-shadow-high.bin","adc-input-global-inputs.bin","adc-input-pulls.toml","userdata-staging.toml","source/adc-input-capture-decision.toml","source/adc-parameter-capture.h","adc-parameter-static/scope.toml","adc-parameter-static/main-grammar.toml","adc-parameter-static/stary-grammar.toml","adc-parameter-static/stary-refinements.toml","adc-parameter-static/integer-refinements.toml","adc-parameter-static/helper-grammar.toml","adc-parameter-static/helper-refinements.toml","adc-parameter-static/getter-inputs.toml","native-helper-status.toml","probe-result.toml","loader-before-removal-package.txt","native-events.toml","native-status.toml","group-control.elf","group-executed.elf","binary-sha256.txt","build-inputs.txt","installed.apk","adc-stock.bin","spu-stock.bin","reference.tsv","tail-candidate.toml","tail-grammar.toml","tail-control-fixture.toml","tail-handoff-profile.toml","tail-stock-gate.toml","calibration-loader-candidate.toml","source/calibration-stock-inputs.toml","source/group-observer.c","source/calibration-loaders.h","source/VerifySnapshot.main.kts","source/VerifyNative.main.kts","source/VerifyCalibrationFilesystem.main.kts","loader-command-status.toml","native-loader-pulls.toml","loader-fixture-verification.toml","loader-labelled-labels.txt","loader-labelled-stat.txt","loader-labelled-absent.txt","loader-final-labels.txt","loader-final-stat.txt","loader-final-absent.txt","loader-final-lsb.bin","loader-final-vertical.bin","native-enforcing.txt","native-final-enforcing.txt","final-enforcing.txt","loader-final-package.txt","final-packages.txt","final-processes.txt","system-server-pid.toml","fixture-system-server.toml","native-final-system-server.txt","final-system-server.txt","final-health-status.toml","result.toml","fixture-before-mounts.txt","fixture-before-mountinfo.txt","fixture-before-stat.txt","fixture-before-labels.txt","fixture-before-empty.txt","fixture-before-pid.txt","fixture-after-root-pid.txt","fixture-enforcing.txt","fixture-processes-before.txt","fixture-packages-before.txt","fixture-command-status.toml","fixture-after-mounts.txt","fixture-after-mountinfo.txt","fixture-after-stat.txt","fixture-after-labels.txt","fixture-absent-paths.txt","calibration-lsb-stock.bin","calibration-lsb-roundtrip.bin","calibration-vertical-stock.bin","calibration-vertical-roundtrip.bin","fixture-result.toml","fixture-ramdisk.img")
    require((consumed+runtimePins.keys.map{"source/$it"}).all{run.resolve(it).toAbsolutePath().normalize() in indexed})
}
verify()
println("schema_version = \"mho900-lab.stock-adc-input-capture-verification/1\"")
println("result = \"accepted\"")
println("mode = \"adcinputmodel\"")
