// Independent evidence verification for the full ADC transcript and private observer controls.
...
require(args.size in 1..3) { "Usage: VerifyAdcObserver.main.kts RUN_DIRECTORY [13..24|transcript-stock] [ADMISSION_VERIFIER_OVERRIDE]" }
val run=Path.of(args[0]).toAbsolutePath().normalize()
val stock=args.getOrNull(1)=="transcript-stock"
val stockPair=false
...
val arms=if(stock) listOf(-1) else if(args.size==2) listOf(args[1].toInt().also { require(it in 13..24) }) else (13..24).toList()
for(arm in arms) {
    val pairWrite=false
    val oneWrite=true
    val continuation=true
    val expectedWrites=if(stock || arm in 13..14) 452 else if(arm in 15..16 || arm==18) 1 else 0
    val fixtureName=if(stock) "adc-stock.bin" else if(arm==18) "adc-private-one.bin" else "adc-private.bin"
    ...
    val writeLimit=input32(24).toInt()
    ...
    require(inputEvent.u("write_count")==writeLimit.toULong() && inputEvent.u("total_size")==fixtureBytes.size.toULong())
    ...
    val mode=es[2]; require(mode.kind=="model-mode" && mode.s("scope")==if(stock) "stock" else "private")
    require(mode.u("transcript")==1uL && mode.u("profile")==inputEvent.u("profile") && mode.u("write_count")==writeLimit.toULong())
    ...
    val setup=es[3]; require(setup.kind=="group-setup" && setup.u("options")==0x100009uL)
    ...
    val pre=es.subList(4,coveredAt)
    ...
    while(at<es.size && es[at].kind!="thread-status") {
        ...
        when(e.kind) {
            ...
            "transcript-complete"->{ require(writes==writeLimit && !transcriptComplete && e.u("writes")==writes.toULong()); transcriptComplete=true }
            ...
            "mapped-fault"->{
                ...
                if(count==0) { require(r.tid==pid && r.pc==binding.u("first_pc") && e.u("offset")==0x4048uL); firstFault=r }
                    ...
                    if(!stock) {
                        val worker=arm==19 || (arm==14 && writes==452)
                        val readTerminal=arm in 13..14 && writes==452
                        val badOffset=arm==16 && writes==1
                        require(r.x[8]==e.u("address") && e.u("offset")==if(readTerminal) 0x4040uL else if(badOffset) 0x3004uL else 0x3000uL)
                        require(r.tid==if(worker) mode.u("fixture_worker") else pid)
                        require(r.pc==if(readTerminal && worker) binding.u("worker_pc") else if(readTerminal) binding.u("second_pc") else if(arm==20) binding.u("write64_pc") else binding.u("write_pc"))
                        require(e.u("opcode")==if(readTerminal) 0xb9400109uL else if(arm==20) 0xf9000109uL else 0xb9000109uL)
                        if(!readTerminal) require(r.x[9] and 0xffffffffuL==if(arm==15 && writes==1) oracle[1] xor 1uL else if(arm==17) oracle[1] else oracle[writes])
                    }
            ...
            "modeled-write"->{
                ...
                val expectedWord=oracle[writes]
                require(f.u("tid")==pid && before.tid==pid && f.u("offset")==0x3000uL && before.x[8]==f.u("address") && before.x[9] and 0xffffffffuL==expectedWord)
                require(before.pc==binding.u("write_pc") && f.u("opcode")==0xb9000109uL)
                require(e.u("tid")==pid && e.u("index")==writes.toULong() && e.u("offset")==0x3000uL && e.u("value")==expectedWord && e.u("width")==4uL)
                val claim=take("transcript-write")
                require(claim.u("index")==writes.toULong() && claim.u("tid")==pid && claim.u("pc")==before.pc && claim.u("opcode")==0xb9000109uL && claim.u("offset")==0x3000uL && claim.u("value")==expectedWord && claim.u("width")==4uL)
    ...
    if(!stock && arm in 21..24) require(liveState==false && liveRejection!=null)
    else if(stock || arm in 13..18) require(liveState==true)
    require(writeRejection!=null || liveRejection!=null)
    require(transcriptComplete==(writes==writeLimit))
    for(i in 0..1) {
        val shadow=take("transcript-shadow")
        val expected=input32((if(writes==452) 1256 else 1252)+32*i)
...
if(args.size==1 || stock) {
    ...
    if(stock) {
        require(text("result.toml").contains("mode = \"transcriptmodel\""))
        ...
        require(text("result.toml").contains("mode = \"transcriptcontrol\"") && text("result.toml").contains("inspection = \"completed\""))
